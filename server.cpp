...
// reading data point request
void process_data_request (FIFORequestChannel* rc, char* request) {
	datamsg* d = (datamsg*) request;
	// look at patient file 1, at specific time of either the first or second ecg column
	double data = get_data_from_memory(d->person, d->seconds, d->ecgno);
	// writes data point stored in data with exactly that many bytes
